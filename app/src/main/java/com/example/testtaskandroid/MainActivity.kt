...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        appBarConfiguration = AppBarConfiguration(setOf(
            R.id.nav_quotes, R.id.nav_products, R.id.nav_settings), drawerLayout)
        setupActionBarWithNavController(navController, appBarConfiguration)
        navView.setupWithNavController(navController)

        if (ConfigureSettings.configureSettingsInfo == null
            || (ConfigureSettings.configureSettingsInfo?.firstParam!!.isEmpty()
                && ConfigureSettings.configureSettingsInfo?.secondParam!!.isEmpty()
                && ConfigureSettings.configureSettingsInfo?.thirdParam!!.isEmpty())) {
